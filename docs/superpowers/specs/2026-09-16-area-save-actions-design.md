...
2. **Recharge is in scope, not deferred.** 30 of the 32 recognised actions
   carry a recharge limit; without it a dragon could breathe every turn,
   which is not the SRD rule and would make these enemies far deadlier than
   intended.
...
- **81 actions across the roster carry a `dc` field** (a saving throw). Of
  these:
  - **36** deal damage;
  - **44** apply only a condition, no damage (Frightful Presence, Enslave, …);
  - **1** is a rider on an already-recognised weapon attack (the aboleth's
    Tentacle).
- **Of the 36 damage-dealing actions, 33 share one exact clause**: *"Each
  creature in {area} must make/succeed on a DC {n} {ability} saving throw,
  taking {dice} ({formula}) {type} damage on a failed save, or half as much
  damage on a successful one."* The other 3 (Otyugh's Tentacle Slam, the
  water elemental's Whelm, the vrock's Spores) either apply damage and a
  condition with no "half on success," or deal no immediate damage at all —
  a different template, out of scope.
- **Of those 33, 32 recognise cleanly.** `young-red-dragon`'s Fire Breath
  carries a source-data defect — `"DC l3"` (a lowercase L in place of `13`)
  — and is left unrecognised, the same fail-closed treatment as the 39
  unrecognised weapon attacks from the enemy-turns spec's own measurement.
  **All 32 carry a recharge limit** (`1d6`, minimum 4, 5, or 6); none needs
  an "always available" case.

## 3. Scope

### In

- Verbatim recognition of the 32 area-damage-with-save actions into the
...
Every recognised action's description contains, verbatim up to numeric and
free-text slots:

> *Each creature {in that area | in that line | within N feet of that point}
> must {make | succeed on} a DC {n} {ability} saving throw, taking {x}
> ({dice}) {type} damage on a failed save, or half as much damage on a
> successful one.*

Trailing prose after this clause (one monster's "Being underwater doesn't
grant resistance against this damage.") is ignored, the same way trailing
narrative after an attack's "Hit: …" clause is ignored today.

### 4.2 The shape clause and its reach

The sentence preceding the core clause gives the shape and size. It reduces
to a maximum reach in feet, via a closed table of templates (representative
examples; the implementation task enumerates every exact wording measured):

| Template (slots in braces) | Reach |
| --- | --- |
| "exhales {noun phrase} in a{n} {N}-foot cone." | N |
| "exhales {noun phrase} in a{n} {N}-foot line that is {w} feet/ft. wide." | N |
| "exhales a {N}-foot cone of {noun}." | N |
| "spits/exhales {noun phrase} in a line that is {N} ft. long and {w} ft. wide[, provided …]." | N |
| "hurls a magical lightning bolt at a point it can see within {N} feet of it." | N |

`a`/`an` is accepted either way — the source data has at least one "an
60-foot line" typo, and the recogniser does not fail an otherwise-exact
match over an article.

### 4.3 The persisted shape
...
### 4.4 Saving-throw proficiency by class
...
1. The player's save modifier: `abilityModifier(stats[saveAbility]) +
   (proficient ? proficiencyBonus(level) : 0)`, using §4.4's table. This
...
monsters, roughly 30 gain `areaSaveAttack`; every other monster's profile is
untouched and the planner's new tiers never trigger for it.

## 8. Testing

- **Recognition**, measured against `data/srd-es/monsters.json`: the 32
  recognised actions match exactly; `young-red-dragon`'s Fire Breath stays
  unrecognised (the `"DC l3"` defect); each shape template has its own case;
  the `a`/`an` article variance is covered.
