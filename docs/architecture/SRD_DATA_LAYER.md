...
| Prisma SRD tables — monsters/spells | `SrdMonster` and `SrdSpell` in `prisma/schema.prisma`; read by `lib/ai/tools/srd-lookup.ts`; populated through transitional paths including `prisma/seed-srd.ts` from `data/srd-es/` JSON and legacy `scripts/ingest-srd.ts` from `docs/reference/srd/` Markdown | Local searchable cache and typed projection for backend tools. | Derived cache, never independent canon. |
| Prisma SRD tables — generic items | `SrdItem` in `prisma/schema.prisma`; read by `lib/ai/tools/srd-lookup.ts` and by `lib/rules/srd-equipment-lookup.ts` (its primary mechanical reader); can receive equipment JSON from `prisma/seed-srd.ts` via `data/srd-es/equipment.json` | Generic local item cache for raw equipment/item payloads. | Derived cache, never independent canon. |
| Prisma SRD tables — typed equipment | `SrdEquipment` in `prisma/schema.prisma`; has no reader and no writer anywhere in the codebase; do not assume it is populated by `prisma/seed-srd.ts` | Unused table, seeded with zero rows, awaiting a retirement decision. Dropping it is deliberately out of scope for now. | Derived cache, never independent canon. |
| Prisma SRD tables — conditions | `SrdCondition` in `prisma/schema.prisma`; seeded by `scripts/seed-conditions.ts` from `https://www.dnd5eapi.co/api/2014/conditions` | Canonical 5e/SRD condition registry cache used to validate condition application. | Derived cache, never independent canon. |
