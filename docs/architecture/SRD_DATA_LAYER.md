...
| Prisma SRD tables — generic items | `SrdItem` in `prisma/schema.prisma`; read by `lib/ai/tools/srd-lookup.ts`; can receive equipment JSON from `prisma/seed-srd.ts` via `data/srd-es/equipment.json` | Generic local item cache for raw equipment/item payloads. | Derived cache, never independent canon. |
| Prisma SRD tables — typed equipment | `SrdEquipment` in `prisma/schema.prisma`; read by `lib/ai/tools/srd-lookup.ts`; do not assume it is populated by `prisma/seed-srd.ts` | Typed equipment projection for weapon/armor/equipment lookups when populated by an explicit adapter or sync path. | Derived cache, never independent canon. |
